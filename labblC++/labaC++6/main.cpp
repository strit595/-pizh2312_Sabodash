...
int main() {
    ...
    std::cout << "\nИспользование оператора + для добавления контакта:\n";
    справочник + new РабочийКонтакт("Иван", "+7-999-234-56-78", "ул. Мира, 10", "Менеджер", "ООО Ромашка");
    справочник + new ЛичныйКонтакт("Мария", "+7-999-345-67-89", "пр. Победы, 5", "01.01.1990", "maria@example.com");

    // Вывод с использованием оператора <<
    std::cout << "\nВывод справочника с помощью оператора <<:\n";
    std::cout << справочник;

    // Демонстрация префиксного оператора ++
    std::cout << "\nИспользование префиксного оператора ++ (добавляет РабочийКонтакт):\n";
    ++справочник;
    std::cout << справочник;

    // Демонстрация постфиксного оператора ++
    std::cout << "\nИспользование постфиксного оператора ++ (добавляет ЛичныйКонтакт):\n";
    справочник++;
    std::cout << справочник;

    // Демонстрация оператора []
    std::cout << "\nИспользование оператора [] для доступа к элементам:\n";
    for (size_t i = 0; i < справочник.getКонтакты().size(); ++i) {
        std::cout << "Контакт " << i << ":\n";
        if (справочник[i]) {
            справочник[i]->вывестиВсе();
            std::cout << "-------------------" << std::endl;
        }
    }

    // Демонстрация доступа к несуществующему индексу
    std::cout << "\nПопытка доступа к несуществующему индексу:\n";
    справочник[10]; // Ожидается сообщение об ошибке
